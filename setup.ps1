...
if (-not (Test-Path ".env")) {
    ...
    if (Test-Path ".env.example") {
        ...
        Write-Host "Please edit .env with your API keys before using the server" -ForegroundColor Yellow
    } else {
...
if (Test-Path $cursorConfigDir) {
    Write-Host "Installing MCP configuration for Cursor..." -ForegroundColor Cyan
    if (Test-Path ".cursor\mcp.json") {
        Copy-Item ".cursor\mcp.json" "$cursorConfigDir\mcp.json" -Force
        $cursorConfigInstalled = $true
    } else {
        Write-Host ".cursor\mcp.json not found, creating basic config..." -ForegroundColor Yellow
        @"
{
  "mcpServers": {
    "domain-finder": {
...
"@ | Out-File -FilePath "$cursorConfigDir\mcp.json" -Encoding UTF8
    ...
    Write-Host "Cursor config directory not found, skipping Cursor setup" -ForegroundColor Yellow
...
Write-Host ""
Write-Host "MCP Server Settings:" -ForegroundColor Cyan
